export default (request, response) => {
  ...
  controllers.user_read.post = () => {
    let chanEverybodyR = [body.global + "#user:" + body.uuid + "#read.*"];
  ...
  controllers.user_write.post = () => {
    let chanEverybodyW = [body.global + "#user:" + body.uuid + "#write.*"];
  ...
  controllers.join.post = () => {
    let group = encodeURIComponent(
      [body.global, body.uuid, body.chat.group].join("#")
    );
  ...
  controllers.chat.post = () => {
    return db
      .set("meta:" + body.chat.channel, body.chat, 525600)
      .then(() => {
        return response.send();
      })
      .catch(() => {
  ...
  controllers.chat.get = () => {
    return db
      .get("meta:" + request.params.channel)
      .then(value => {
        if (value) {
          return response.send({
            found: true,
